//! Real no_std compatibility test
//!
//! This test verifies that the library's public API works correctly for no_std use cases.
//! While the test itself runs with std (required by Rust's test framework), it exercises
//! all code paths that would be used in actual no_std environments.
//!
//! To verify actual no_std compilation, use the build verification script:
//!   ./scripts/verify_no_std.sh
//!
//! Or manually:
//!   cargo build --lib --no-default-features            # Core only
